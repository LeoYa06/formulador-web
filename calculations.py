...
def process_ingredients_for_display(ingredients_data: list[dict]) -> list[dict]:
    ...
    # 1. Mapeo de categorías a un número de orden.
    CATEGORY_ORDER = {
        "Cárnico": 1,
        "Agua/Hielo": 2,
    ...
    for ing in ingredients_data:
        ...
        # --- ¡LÍNEAS RESTAURADAS! ---
        # Volvemos a calcular los kg de cada nutriente por ingrediente
        ...
        processed_data.append({
            ...
            'kg_total': kg_total,
            'kg_protein': kg_protein, # <-- DATO RESTAURADO
            'kg_fat': kg_fat,         # <-- DATO RESTAURADO
            'kg_water': kg_water,       # <-- DATO RESTAURADO
...
def calculate_formula_totals(processed_ingredients: list[dict]) -> dict:
    ...
    # --- ¡CÁLCULOS DE RATIOS RESTAURADOS! ---
    # Usamos los porcentajes que acabamos de calcular
    ...
    return {
        ...
        'aw_fp_ratio_str': aw_fp_ratio_str, # <-- DATO RESTAURADO
        'af_fp_ratio_str': af_fp_ratio_str  # <-- DATO RESTAURADO
    }